...
def CheckAssemblyDummyFiles() -> None:
...
def NewMethod(partsToSave: set[Part], child: Component) -> None:
    # Part proto = (Part)child.Prototype
...
def ExportPDF(outPutPath: str, fileName: str) -> str:
    # DrawingSheet[] sheets = session_.Parts.Work.DrawingSheets.ToArray()
...
def AddDummy(part: Part, layers: Sequence[int]) -> None:
...
def SetLayersInBlanksAndLayoutsAndAddDummies(snapStrip010: Part) -> None:
